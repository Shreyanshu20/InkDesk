    ...
    const fetchSubcategories = async () => {
      ...
      try {
        ...
        if (!data.categories || !Array.isArray(data.categories)) {
          throw new Error(
            "Invalid response format: categories array not found"
          );
        }
        ...
        if (
          ...
          !Array.isArray(targetCategory.subcategories)
        ) {
          setSubcategories([]);
          setLoading(false);
          return;
        ...
        const mappedSubcategories = targetCategory.subcategories.map(
          (subcat, index) => {
            return {
              id: subcat._id,
              ...
              hasValidImage: !!(
                subcat.subcategory_image &&
                subcat.subcategory_image.trim() !== ""
              ),
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="text-center py-12">
            ...
            <p className="text-text/70">
              No {category} subcategories available yet.
            </p>
            <p className="text-text/50 text-sm mt-2">
              Add some subcategories in the admin panel to see them here.
            </p>
          </div>
        </div>
      </section>
    ...
    <section className="py-10 px-4 md:px-6 bg-background">
      <div className="container mx-auto max-w-7xl">
        <div className="text-center mb-8 md:mb-12 lg:mb-16">
          ...
          <div className="w-24 h-1 bg-primary mx-auto mt-4"></div>
          {description && (
            <p className="text-text/70 mt-4 max-w-2xl mx-auto">{description}</p>
          )}
        </div>
